...
if search_value:
    if search_type == "NDC":
        ndc_filtered_df = df[df['NDC'].astype(str).str.contains(search_value, na=False)]
        if not ndc_filtered_df.empty:
            st.subheader(f"Details for NDC: {search_value}")
            for _, row in ndc_filtered_df.iterrows():
                st.markdown("---")
                st.markdown(f"### Drug Name: **{row['Cleaned Up Drug Name']}**")
                st.markdown(f"- **Insurance**: {row['Insurance']}")
    ...
    st.info("Start typing an NDC to see matching results.")

# Filter data based on both search and insurance if applicable
...
# Display results for Drug Name or Rxcui search
if not filtered_df.empty:
